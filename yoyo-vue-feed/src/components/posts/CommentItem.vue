<template>
    <ion-item>
      ...
      <ion-label>
        ...
        <div class="translate" @click="handleTranslate(comment)">translate comment</div>
  ...
  defineProps<{ comment: ICommentItem }>();
  
  const handleTranslate = async(comment: ICommentItem) => {
    showTranslated.value = !showTranslated.value;
  if (!comment?.translatedContent) {
    await translate(comment, comment.content, 'en', 'ko');  // translate post comment
  }
  };
  </script>
